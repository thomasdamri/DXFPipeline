...
def _count_tiles(w: int, h: int, max_zoom: int) -> int:
    short = min(w, h)
    return sum(
        max(1, math.ceil(w * (1 << z) / short)) *
        max(1, math.ceil(h * (1 << z) / short))
...
def _generate_tiles(img: Image.Image, out_dir: Path, max_zoom: int, tile_sz: int):
    ...
    for z in range(max_zoom + 1):
        ...
        logger.debug("z=%d  %3dx%-3d tiles  [%d/%d  %.0f%%]",
                     z, cols, rows, written, total_tiles, 100 * written / total_tiles)
...
def main():
    ...
    logger.info("Max zoom     : %d  (grid %dx%d, target %d x %d px)",
                max_zoom, cols_max, rows_max, full_w_px, full_h_px)

    svg_bytes = Path(args.svg).read_bytes()
    Image.MAX_IMAGE_PIXELS = full_w_px * full_h_px + 1  # silence decompression-bomb guard for large diagrams
    ...
    if n_strips == 1:
        ...
        logger.info("Rasterising  : %s -> %d x %d px in %d strips ...",
                    args.svg, full_w_px, full_h_px, n_strips)
        ...
        for i in range(n_strips):
            ...
            strip_img = _png_to_image(
                cairosvg.svg2png(bytestring=patched, output_width=strip_w_px, output_height=full_h_px)
            )
    ...
    logger.info("Done: %d x %d px  |  zoom 0-%d  |  %d tiles  |  %s/",
