...
def _apply_theme(doc: Any, theme_cfg: ThemeConfig) -> None:
...
def _make_settings(text_to_path: bool) -> Settings:
    ...
    try:
        ...
        logger.debug("text-to-path: using TextPolicy.FILLING")
    ...
    if hasattr(s, "show_text"):
        ...
        logger.warning("text-to-path not supported by this ezdxf version "
                       "-- text will remain as <text> elements")
    return s


def _render_one(dxf_path: str, svg_out: str, theme_cfg: ThemeConfig | None, text_to_path: bool) -> None:
    ...
    logger.debug("DXF extents: x=[%.4f, %.4f]  y=[%.4f, %.4f]",
                 dxf_x_min, dxf_x_max, dxf_y_min, dxf_y_max)
