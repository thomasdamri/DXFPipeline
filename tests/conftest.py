...
def full_test_dxf():
    """Path to the generated test_diagram.dxf.  Skips if not present."""
    p = TESTS_DIR / "test_diagram.dxf"
    if not p.exists():
        pytest.skip("test_diagram.dxf not found — run tests/generate_test_dxf.py first")
    return p


@pytest.fixture(scope="session")
def test_labels_path():
    """Path to test_labels.txt.  Skips if not present."""
    p = TESTS_DIR / "test_labels.txt"
    if not p.exists():
        pytest.skip("test_labels.txt not found — run tests/generate_test_dxf.py first")
    return p
